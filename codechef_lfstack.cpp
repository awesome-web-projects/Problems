...
bool map_record_state_moves(long long int now, std::vector<long long int>& state) {
	...
	if (now == sum) {
		// check if all stacks are empty, it should have been empty by now, since we are poping elements on a match.
		forl(j, 0, state.size()) {
	...
	/** Long story mode now (:->)
		What we actaully want to see is that given a number from the seqeunce, do we get a way to produce that number
		at that given position by poping elements from the stacks as done by the threads. Now, we may be able to produce the 
		number by the current no of elements that have been poped till now or by poping a new element and seeing if that 
		helps in making the sequence till the number we consider in at the given state. We intially consider that it is not
		possible. When there are more than one ways of making a sequence by poping from one or more stacks, there is no method to tell
		as to which stack to always choose greedily. We thus have to go for a dynamic programming approach.
	*/
	bool current = false;
	// For all the stacks available, we consider the top element of each stack right now and process as explained above.
	forl(i, 0, state.size()) {
		if (stacks[i][state[i]] == sequence[now]) {
			// Pop element.
			state[i]--;
			current = map_record_state_moves(now + 1, state);
			// Consider by not poping it. Similar to what we did in Subset Sums dynamic programming.
			state[i]++;
			if (current) break;
		}
	}
...
void solve_problems()
{
	...
	while (T--) {
		sum = 0LL;
		std::cin >> N;
		// states[i] records the number of intergers left to be considered in a given stack i. 
		...
		if ((N != 1) && map_record_state_moves(0, states)) {
			...
			// Only one thread, just reverse the given sequence and check if it matches the stack push order.
