...
void solve_problems() {
  ...
  /* Edges are in the order of N * K since, the number of edges from a given
  Node say 'a' is atmost K. N nodes, K per node. */
  ...
  /* Since we need to only need the values in the range i to i + K to process a
   * node 'i', we use a min_heap with K elements and find find the min in O(1)
   * time extraction and insertion are order log(K) operations. */
  ...
  for (auto i = 1; i < loop; ++i) {
    while ((i - min_heap.top().second) > K)
      min_heap.pop();                // We don't need the older nodes actually.
    auto min_value = min_heap.top(); // Min element at top of min heap.
    process_dag_dp[i] = process_dag_dp[min_value.second] +
                        log(A[i]); // We start be taking the min value upto now,
                                   // i to i + K range.
    min_till_now[i] = (min_till_now[min_value.second] * 1LL * A[i]) % modulo;
    /* Now adding this value to priority queue, to get min value aftwerwards for
     * some other node 'j' */
